...
def non_maximum_suppression(bbox, thresh, score=None,#根据传入的box 和score 规定的thresh 计算返回nms处理后选择的box索引（按照分数从高到低排序）
                            limit=None):
    ...
    如果边界框和先前选择的边界框之间的并集交点（IoU）小于thresh，则此方法会顺序检查每个边界框并选择边界框。 该方法主要用作对象检测的后处理。
    ...
    边界框是从得分较高的框中选择的。
   如果未提供：obj：`score`作为参数，则边框按其索引升序排列。
    ...
    预计边框将打包成二维（R，4）形状的张量，其中：R：是图像中边界框的数量。 第二个轴表示边界框的属性。 它们是（y_ {min}，x_ {min}，y_ {max}，x_ {max}）`，其中四个属性是左上和右下顶点的坐标。
    ...
    the same type.#bbox score==(35*50*9,4)
...
def _non_maximum_suppression_gpu(bbox, thresh, score=None, limit=None):
    ...
    n_bbox = bbox.shape[0]#(35*50*9)

    if score is not None:
        order = score.argsort()[::-1].astype(np.int32)#将score降序排序
    else:
...
def _call_nms_kernel(bbox, thresh):
    ...
    threads = (threads_per_block, 1, 1)#(64,1,1)
