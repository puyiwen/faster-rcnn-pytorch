...
class ProposalTargetCreator(object):#从2000个ROIs中筛选出128个用于训练
    ...
    def __call__(self, roi, bbox, label,
        ...
        * :math:`L` is number of object classes possibly including the \
        ...
        """
        n_bbox, _ = bbox.shape#(R1,4)

        roi = np.concatenate((roi, bbox), axis=0)#(300+R1,4)

        pos_roi_per_image = np.round(self.n_sample * self.pos_ratio)#128*0.25=32 正样本
...
class AnchorTargetCreator(object):#从35×50×9个anchors中筛选256个anchors进行RPN网络训练
    ...
    Args:
        n_sample (int): The number of regions to produce. 区域数
        pos_iou_thresh (float): Anchors with IoU above this
            threshold will be assigned as positive.
        neg_iou_thresh (float): Anchors with IoU below this
            threshold will be assigned as negative.
        pos_ratio (float): Ratio of positive regions in the
            sampled regions. 采样区域中正区域的比率
    ...
    def __call__(self, bbox, anchor, img_size):
        ...
        * :math:`R` is the number of bounding boxes.
        ...
        img_H, img_W = img_size#(35*50)

        n_anchor = len(anchor)#35*50*9
        inside_index = _get_inside_index(anchor, img_H, img_W)##将那些超出图片范围的anchor全部去掉,只保留位于图片内部的序号
        anchor = anchor[inside_index]#记录挑选下来的anchor索引
        argmax_ious, label = self._create_label(
            inside_index, anchor, bbox)#筛选出符合条件的正例128个负例128并给它们附上相应的label

        # compute bounding box regression targets 计算边界框回归
        loc = bbox2loc(anchor, bbox[argmax_ious])#计算anchor和自己对应IOU最大的gt_bbox的损失

        # map up to original set of anchors
        label = _unmap(label, n_anchor, inside_index, fill=-1)#将位于图片内部的框的label对应到所有生成的框中
        loc = _unmap(loc, n_anchor, inside_index, fill=0)##将回归的框对应到所有生成的框中

        return loc, label #(35*50*9,4) (35*50*9,1) only 128=1,128=0 The remaining=-1

    def _create_label(self, inside_index, anchor, bbox):
        # label: 1 is positive, 0 is negative, -1 is dont care
        label = np.empty((len(inside_index),), dtype=np.int32)#inside_index为所有在图片范围内的anchor序号
        label.fill(-1)#fill all -1
...
class ProposalCreator:
    ...
    def __call__(self, loc, score,
                 anchor, img_size, scale=1.):#传入，预测的loc，score分数，featuremap的所有anchor
