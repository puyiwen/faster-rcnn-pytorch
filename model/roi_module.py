...
class RoI(Function):
    def __init__(self, outh, outw, spatial_scale):
        with torch.no_grad():
            ...
            self.outh, self.outw, self.spatial_scale = outh, outw, spatial_scale#7,7,1/16

    def forward(self, x, rois):
        # NOTE: MAKE SURE input is contiguous too
        x = x.contiguous()# #变成在内存中连续分布的形式
        rois = rois.contiguous()
        self.in_size = B, C, H, W = x.size()#(1,512,35,50) featuremap
        self.N = N = rois.size(0)#每张图所有的anchors数 tois:300
        output = t.zeros(N, C, self.outh, self.outw).cuda()#(300,512,7,7)
    ...
    def backward(self, grad_output):#反向传播过程 为了验证cuda code正确性
        ...
        B, C, H, W = self.in_size#(1,512,35,50)
        grad_input = t.zeros(self.in_size).cuda()#(1,512,35,50)
        stream = Stream(ptr=torch.cuda.current_stream().cuda_stream)
...
class RoIPooling2D(t.nn.Module):
    ...
    def __init__(self, outh, outw, spatial_scale):#7,7,1/16
    ...
    def forward(self, x, rois):
        return self.RoI(x, rois)#(300,512,7,7)
